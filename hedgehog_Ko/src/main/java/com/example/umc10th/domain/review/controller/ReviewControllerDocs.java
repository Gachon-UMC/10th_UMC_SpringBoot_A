...
public interface ReviewControllerDocs {
    ...
    ApiResponse<ReviewResDTO.StoreReviewListDTO> getStoreReviews(
            ...
            @Parameter(description = "다음 페이지 조회를 위한 cursor 값. 첫 조회 시 생략합니다.", example = "15")
            Long cursor,

            @Parameter(description = "한 번에 조회할 리뷰 개수", example = "10")
            Integer size
    );
    ...
    ApiResponse<ReviewResDTO.CreateReviewResultDTO> createReview(
            @Parameter(description = "리뷰를 작성할 가게 ID", example = "1")
            Long storeId,
    ...
    @Operation(
            summary = "내 리뷰 목록 조회",
            description = "현재 사용자가 작성한 리뷰 목록을 cursor 기반 페이지네이션으로 조회합니다. cursor를 생략하면 첫 페이지를 조회하고, 응답의 nextCursor는 다음 페이지 요청에 사용합니다. hasNext가 false이면 다음 페이지가 없으며, size는 한 번에 조회할 데이터 개수입니다. sort는 id 또는 starRate 기준 정렬에 사용합니다."
    )
    ApiResponse<ReviewResDTO.MyReviewListDTO> getMyReviews(
            @Parameter(description = "다음 페이지 조회를 위한 cursor 값. 첫 조회 시 생략합니다.", example = "20")
            Long cursor,

            @Parameter(description = "한 번에 조회할 리뷰 개수", example = "10")
            Integer size,

            @Parameter(description = "정렬 기준. id 또는 starRate를 사용할 수 있습니다.", example = "id")
            String sort
    );
